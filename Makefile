...
# If OBJS is set from command line (e.g., to specific .c files), use those
ifndef OBJS
    # Default: build all .c files
    OBJS = $(patsubst %.c,obj/%.o,$(filter %.c,$(SRC)))
else
    ifeq ($(filter %.c, $(OBJS)), $(OBJS))
        # OBJS contains .c files, filter to existing files and convert to .o files
        EXISTING_C = $(wildcard $(OBJS))
        override OBJS := $(patsubst %.c,obj/%.o,$(EXISTING_C))
    endif
endif

# Project target defined by PROJECT_NAME
$(PROJECT_NAME): $(OBJS)
	@if [ -z "$(OBJS)" ]; then echo "No source files to build"; exit 1; fi
